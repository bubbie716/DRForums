...
export function ForumRow({ forum }: ForumRowProps) {
  ...
  return (
    <div className="group grid grid-cols-12 gap-4 px-7 py-6 border-b border-border/60 last:border-b-0 bg-white hover:bg-hover transition-all duration-200">
      <div className="col-span-12 sm:col-span-5 flex items-start gap-4 min-w-0">
        ...
        <div className="min-w-0 flex-1">
          <Link
            ...
            className="forum-title-link inline-block font-bold text-text-dark text-base transition-colors duration-200 hover:text-accent"
          >
            {forum.name}
          </Link>
          {forum.description && (
          ...
          <div className="flex gap-6 mt-3 sm:hidden text-xs text-text-secondary">
            <span>
      ...
      <div className="col-span-12 sm:col-span-5 flex sm:items-center min-w-0">
        {latestPost ? (
          <div className="min-w-0 w-full sm:text-right">
            <Link
              href={`/thread/${latestPost.thread.id}#post-${latestPost.id}`}
              className="forum-title-link inline-block w-full sm:w-auto text-sm font-semibold text-text-dark transition-colors duration-200 hover:text-accent line-clamp-1"
            >
          ...
          <span className="text-sm text-text-muted w-full sm:text-right italic">
            No posts yet
          </span>
        )}
      </div>
